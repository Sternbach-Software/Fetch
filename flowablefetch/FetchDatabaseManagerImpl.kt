...
class FlowableFetchDatabaseManagerImpl constructor(context: Context,
    ...
    FlowableFetchDatabaseManager<DownloadInfo> {
    ...
    private val requestDatabaseFlowable: flowablefetch.DownloadDatabase
    private val database: SupportSQLiteDatabase

    init {
        val builder = Room.databaseBuilder(context, flowablefetch.DownloadDatabase::class.java, "$namespace.db")
        builder.addMigrations(*migrations)
        requestDatabaseFlowable = builder.build()
        database = requestDatabaseFlowable.openHelper.writableDatabase
    }

    override fun insert(downloadInfo: DownloadInfo): Pair<DownloadInfo, Boolean> {
        throwExceptionIfClosed()
        val row = requestDatabaseFlowable.requestDao().insert(downloadInfo)
        return Pair(downloadInfo, requestDatabaseFlowable.wasRowInserted(row))
    }

    override fun insert(downloadInfoList: List<DownloadInfo>): List<Pair<DownloadInfo, Boolean>> {
        throwExceptionIfClosed()
        val rowsList = requestDatabaseFlowable.requestDao().insert(downloadInfoList)
        return rowsList.indices.map {
            Pair(downloadInfoList[it], requestDatabaseFlowable.wasRowInserted(rowsList[it]))
        }
    }

    override fun delete(downloadInfo: DownloadInfo) {
        throwExceptionIfClosed()
        requestDatabaseFlowable.requestDao().delete(downloadInfo)
    }

    override fun delete(downloadInfoList: List<DownloadInfo>) {
        throwExceptionIfClosed()
        requestDatabaseFlowable.requestDao().delete(downloadInfoList)
    }

    override fun deleteAll() {
        throwExceptionIfClosed()
        requestDatabaseFlowable.requestDao().deleteAll()
        logger.d("Cleared Database $namespace")
    }

    override fun update(downloadInfo: DownloadInfo) {
        throwExceptionIfClosed()
        requestDatabaseFlowable.requestDao().update(downloadInfo)
    }

    override fun update(downloadInfoList: List<DownloadInfo>) {
        throwExceptionIfClosed()
        requestDatabaseFlowable.requestDao().update(downloadInfoList)
    }
    ...
    override fun updateExtras(id: Int, extras: Extras): DownloadInfo? {
        ...
        database.setTransactionSuccessful()
        database.endTransaction()
        val download = requestDatabaseFlowable.requestDao().get(id)
        sanitize(download)
        return download
    }
    ...
    override fun get(): List<DownloadInfo> {
        throwExceptionIfClosed()
        val downloads = requestDatabaseFlowable.requestDao().get()
        sanitize(downloads)
        return downloads
    }
    ...
    override fun get(ids: List<Int>): List<DownloadInfo?> {
        throwExceptionIfClosed()
        val downloads = requestDatabaseFlowable.requestDao().get(ids)
        sanitize(downloads)
        return downloads
    }
    ...
    override fun getByFile(file: String): DownloadInfo? {
        throwExceptionIfClosed()
        val download = requestDatabaseFlowable.requestDao().getByFile(file)
        sanitize(download)
        return download
    }
    ...
    override fun getByStatus(status: Status): List<DownloadInfo> {
        throwExceptionIfClosed()
        var downloads = requestDatabaseFlowable.requestDao().getByStatus(status)
    ...
    override fun getByStatus(statuses: List<Status>): List<DownloadInfo> {
        throwExceptionIfClosed()
        var downloads = requestDatabaseFlowable.requestDao().getByStatus(statuses)
        if (sanitize(downloads)) {
    ...
    override fun getByGroup(group: Int): List<DownloadInfo> {
        throwExceptionIfClosed()
        val downloads = requestDatabaseFlowable.requestDao().getByGroup(group)
        sanitize(downloads)
        return downloads
    }
    ...
    override fun getDownloadsInGroupWithStatus(groupId: Int, statuses: List<Status>): List<DownloadInfo> {
        throwExceptionIfClosed()
        var downloads = requestDatabaseFlowable.requestDao().getByGroupWithStatus(groupId, statuses)
        if (sanitize(downloads)) {
    ...
    override fun getDownloadsByRequestIdentifier(identifier: Long): List<DownloadInfo> {
        throwExceptionIfClosed()
        val downloads = requestDatabaseFlowable.requestDao().getDownloadsByRequestIdentifier(identifier)
        sanitize(downloads)
        return downloads
    }
    ...
    override fun getPendingDownloadsSorted(prioritySort: PrioritySort): List<DownloadInfo> {
        ...
        var downloads = if (prioritySort == PrioritySort.ASC) {
            requestDatabaseFlowable.requestDao().getPendingDownloadsSorted(Status.QUEUED)
        } else {
            requestDatabaseFlowable.requestDao().getPendingDownloadsSortedDesc(Status.QUEUED)
        }
        if (sanitize(downloads)) {
    ...
    override fun getAllGroupIds(): List<Int> {
        throwExceptionIfClosed()
        return requestDatabaseFlowable.requestDao().getAllGroupIds()
    }

    override fun getDownloadsByTag(tag: String): List<DownloadInfo> {
        throwExceptionIfClosed()
        val downloads = requestDatabaseFlowable.requestDao().getDownloadsByTag(tag)
        sanitize(downloads)
        return downloads
    }
    ...
    override fun getFlow(): Flow<List<DownloadInfo>> {
        throwExceptionIfClosed()
        return requestDatabaseFlowable.requestDao().getFlow().onEach { sanitize(it) }
    }
